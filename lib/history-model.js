...
module.exports.HistoryModel = function(collectionName, options) {
  var indexes = options && options.indexes,
      additionalFields = options && options.fields,
  ...
  if (!(collectionName in historyModels)) {
    ...
    if(additionalFields) {
      additionalFields.forEach(function(f){
        console.log(f);
        fields[f.key] = f.params;
      });
    }

    var schema = new mongoose.Schema(fields, { id: true, versionKey: false });
